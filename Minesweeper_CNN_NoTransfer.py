...
RUN_TITLE = 'CNN_without_transfer_3stage_6x6_345_30000epi_s2epst05epdc09995'
...
def train():
    ...
    for stage_idx, stage in enumerate(TRAINING_STAGES):
        agent = DQNAgent(stage['rows'], stage['cols'],
        ...
        for ep in range(1, stage['episodes'] + 1):
            ...
            for step in range(stage['rows'] * stage['cols'] * MAX_STEPS_MULTIPLIER):
                ...
                # agent.replay()  ## Placing it here causes it to use the replay buffer more often (every single game), drastically increases runtime but also very positive results for learning rate
                s = s2
                total_reward += r
                if done: break
