# Minesweeper DQN with Progressive Training, Per-Stage Exploration, Reward Tracking, and Evaluation
    ...
    def get_state(self):
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                state[1, r, c] = self.flags[r][c]
                state[2, r, c] = self.board[r][c] if self.revealed[r][c] else -2
        return state
...
class SimpleCNN(nn.Module):
    def __init__(self, input_channels, rows, cols):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(input_channels, 16, kernel_size=3, padding=1), nn.ReLU(),
...
class DQNAgent:
    def __init__(self, rows, cols, epsilon_start=1.0, epsilon_decay=0.999):
        self.rows = rows
        self.cols = cols
        self.q_net = SimpleCNN(3, rows, cols)
        self.target_net = SimpleCNN(3, rows, cols)
    ...
    def act(self, state, valid_actions):
        ...
        state_tensor = torch.tensor(state.reshape(1, 3, self.rows, self.cols), dtype=torch.float32)
        with torch.no_grad():
...
def train():
    ...
    for stage_idx, stage in enumerate(TRAINING_STAGES):
        ...
        agent.replay() ##### CHECK RESULTS, I MOVED THIS ONE LOOP OUT (ORIGINALLY SAME LEVEL AS IF-LOOPS)
    ...
    last_stage_win_rates = win_rates_all[stage_switches[-2]:]  # last stage only
