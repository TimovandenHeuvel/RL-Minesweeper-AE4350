# Improved Minesweeper with DQN Agent and Visualization + Stability Fixes
...
class DQNAgent:
    ...
    def replay(self):
        ...
        for s, a, r, s2, done in batch:
            r = np.clip(r, -10, 50)  # reward clipping
            ...
            q[0, idx] = np.clip(target_val, -100, 100)  # Q-value clipping
            states.append(s)
            targets.append(q)
...
def train():
    ...
    for ep in range(1, NUM_EPISODES + 1):
        ...
        if ep % print_rate == 0:
            ...
            print("Q mean:", np.mean(q_vals), "max:", np.max(q_vals), "min:", np.min(q_vals))

        if ep % 100 == 0:  # More frequent target update
    ...
    plt.grid(True)
    plt.show()
    print(f"\nTotal wins: {total_wins:.2f}, Total episodes: {ep}")

    return agent, (first_game_steps, first_game_board), (last_game_steps, last_game_board)


if __name__ == "__main__":
    agent, (first_steps, first_board), (last_steps, last_board) = train()
    print("\nVisualizing First Game:")
    visualize_game(first_steps, first_board, title="First Game Replay")
    print("\nVisualizing Last Game:")
